...
def main():
    ...
    """Strip large constant values from graph_def."""
        strip_def =tf.GraphDef()
            for n0 in graph_def.node:
                m = strip_def.node.add() #pylint: diable=maybe-no-member
                n.MergeFrom(n0)
                if n.op == 'Const':
    ...
    def render_naive(t_obj, img0=img_noise, iter_n=20, step=1.0)
        t_score = tf.reduce_mean(t_obj) # defining the optimization objective
            t_grad = tf.gradients(t_score, t_input)[0] # behold the power of automatice differentiation!
    img = img0.copy()
        for_in range(iter_n):
...
def render_deepdream(t_obj, img0=img_noise,
    ...
    t_grad = tf.gradients(t_score, t_input)[0] # behold the power of automatice differentiation
        ...
        for _ in range(octave_n-1):
            hw = img.shape[:2]
            lo = resizing(img, np.init32(np.float32(hw)/octabe_scale))
            hi = img-resize(lo, hw)
            img = lo
            octaves.append(hi)
    ...
    for _ in range(iter_n):
        ...
        channel = 139 # picking some feature channel to visualize
            ...
            #open image
		img0 = PIL.Image.open('pilatis800.jpg')
